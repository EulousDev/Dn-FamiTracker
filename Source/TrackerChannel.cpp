...
bool CTrackerChannel::IsInstrumentCompatible(int Instrument, inst_type_t Type) const
{
    switch (m_iChip) {
        ...
        case SNDCHIP_EPSM: {
            switch (m_iChannelID) {
            case CHANID_EPSM_SSG1:
            case CHANID_EPSM_SSG2:
            case CHANID_EPSM_SSG3:
                return Type == INST_S5B;
            case CHANID_EPSM_KICK:
            case CHANID_EPSM_SNARE:
            case CHANID_EPSM_CYMBAL:
            case CHANID_EPSM_HIHAT:
            case CHANID_EPSM_TOM:
            case CHANID_EPSM_RIMSHOT:
                return true;
            default: return Type == INST_VRC7;
            }
        }
    }
...
bool CTrackerChannel::IsEffectCompatible(effect_t EffNumber, int EffParam) const
{
    switch (EffNumber) {
        ...
        case EF_PHASE_RESET:
            ...
            if (m_iChip == SNDCHIP_EPSM) return false;
